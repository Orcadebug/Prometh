...
prometh_test_cols=80
prometh_test_rows=24

prometh_read_terminal_size() {
	prometh_screen_cols="$prometh_test_cols"
	prometh_screen_rows="$prometh_test_rows"
}
...
print_render_meta() {
	...
	printf '__META__ %s cols=%s rows=%s layout_show_logo=%s lab_width=%s render_lab_width=%s compact=%s visible=%s content_height=%s\\n' \\
		"$label" "$prometh_screen_cols" "$prometh_screen_rows" "$prometh_screen_layout_show_logo" \\
		"$prometh_screen_layout_lab_width" "$prometh_screen_render_lab_width" "$prometh_screen_compact" "$visible" "$content_height"
}

render_case() {
	prometh_screen_title="Installing Prometh"
	prometh_screen_detail="Fetching the verified package."
	prometh_screen_question=
	prometh_screen_frame=1
	prometh_screen_cols="$1"
	prometh_screen_rows="$2"
	...
	prometh_screen_frame=2
	prometh_screen_cols="$3"
	prometh_screen_rows="$4"
	prometh_refresh_screen_layout_mode
...
screen_case() {
	...
	prometh_screen "Installing Prometh" "Installing Prometh" "Fetching the verified package." ""
...
Finalizing npm install."
	for progress_frame in 1 24 25 48 49 200; do
		prometh_animation_frame="$progress_frame"
		printf '__PROGRESS__ %s\t%s\t%s\\n' "$progress_frame" "$(prometh_animation_status "Installing Prometh" "$progress_details" static)" "$(prometh_animation_detail "$progress_details")"
	done
}
...
const tempDir = mkdtempSync(join(tmpdir(), "prometh-installer-render-"));
...
function assertInstallerProgress(progress) {
	...
	for (const [index, expectedDetail] of expectedDetails.entries()) {
		...
		check(
			progress[index].status === "Installing Prometh...",
