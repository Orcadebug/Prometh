...
```bash
prometh --mode json "Your prompt"
```

Outputs all session events as JSON lines to stdout. Useful for integrating Prometh into other tools or custom UIs.
...
```bash
prometh --mode json "List files" 2>/dev/null | jq -c 'select(.type == "message_end")'
```
