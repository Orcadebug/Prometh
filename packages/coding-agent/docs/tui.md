> Prometh can create TUI components. Ask it to build one for your use case.
...
Prometh shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Prometh is streaming a response.
...
// Restore Prometh's default spinner
ctx.ui.setWorkingIndicator();
```
