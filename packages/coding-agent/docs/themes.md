> Prometh can create themes. Ask it to build one for your setup.
...
Prometh loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.prometh/themes/*.json`
- Project: `.prometh/themes/*.json`
...
On first run, Prometh detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.prometh/themes
vim ~/.prometh/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Orcadebug/Prometh/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Prometh reloads it automatically for immediate visual feedback.
...
Prometh uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Prometh falls back to the nearest approximation.
