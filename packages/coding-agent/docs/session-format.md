...
```
~/.prometh/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.prometh/sessions/`.

Prometh also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Prometh uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Prometh AI)
...
### Extended Message Types (from Prometh Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Prometh (legacy field name)
