...
Prometh requires Node.js 22.8.0 or newer.
...
Prometh is the product, public CLI, release artifact, and repository name. The monorepo still retains inherited `@earendil-works/pi-*` npm workspace names, a source-package `pi` bin entry, the `pi` package manifest key, and some `PI_*` compatibility environment variables. These names are source and compatibility details, not a signal that contributors should install or develop against pi-mono.

Public releases are currently versioned tarball artifacts installed by the stable and beta installer scripts. `scripts/pack-prometh-release.mjs` rewrites the coding-agent package name, executable, config metadata, and internal dependency URLs for that distribution. Do not document the inherited npm workspace package as the public Prometh install path.

## Local Configuration

User configuration lives under `~/.prometh/`. Project-local settings, prompts, themes, extensions, skills, and system-prompt files live under `.prometh/` in the project root. Override the user config directory with `PROMETH_CODING_AGENT_DIR` and the session directory with `PROMETH_SESSION_DIR`.
...
```bash
PROMETH_CODING_AGENT_DIR=/tmp/prometh-dev /path/to/prometh/prometh.sh
```
...
Prometh runs from source, Node.js package output, and standalone release artifacts. Always use `src/config.ts` helpers for package assets:
...
The hidden `/debug` command writes `~/.prometh/prometh-debug.log` with rendered TUI lines, their visible widths, and the current agent messages. Daemon, worker, client, and provider diagnostic logs live under `~/.prometh/logs/`.
