...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Prometh to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.prometh/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
prometh -c                  # Continue most recent session
prometh -r [path|id]        # Browse sessions or resume one directly
prometh --no-session        # Ephemeral mode; do not save
prometh --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `prometh agents`, `prometh list`, or `prometh attach <agent>` to find and reattach to running work. `prometh stop <agent>` stops one root agent, while `prometh shutdown` stops all workers and the local supervisor.
...
- `.prometh/SYSTEM.md` for a project
- `~/.prometh/SYSTEM.md` globally
...
prometh stop <agent>
prometh rename <agent> <name>
prometh send <agent> <message>
prometh schedule <list|add|cancel>
prometh status
prometh doctor [--fix]
prometh shutdown [--force]

prometh package install <source> [--local]
prometh package remove <source> [--local]
prometh package list
prometh package update [source]
prometh update [--force]
prometh config
```

See [Prometh Packages](packages.md) for package sources and security notes.
...
Use `prometh session export <file> [output]` to export a session to HTML.
...
```bash
prometh --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `PROMETH_CODING_AGENT_DIR` | Override config directory; default is `~/.prometh` |
...
| `PROMETH_KERNEL_PYTHON` | Use an existing Python environment with `ipykernel` instead of bootstrapping `~/.prometh/kernel-venv` |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

The remaining `PI_*` variables are compatibility names still read by the current runtime. They do not change the application name, command, or default `~/.prometh` configuration path.

## Design Principles

Prometh keeps the model-facing tool surface small while making the IPython runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and Prometh packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

Prometh preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current Prometh architecture.
