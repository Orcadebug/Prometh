...
Connect external services (Linear, Notion, …) to Prometh over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with Prometh's single-tool design, MCP integrations are **not**
...
Credentials are stored once in `~/.prometh/auth.json` under `mcp:<name>`.
...
Add it under `mcpServers` in `~/.prometh/settings.json` (or project
...
`~/.prometh/skills/acme/`) with the standard Python-skill layout:
...
`pyproject.toml` (depends on `mcp`, `httpx`, and `prometh-runtime`):

```toml
[project]
name = "prometh-skill-acme"
version = "0.1.0"
requires-python = ">=3.10"
dependencies = ["mcp", "httpx", "prometh-runtime"]
...
- **Custom kernel + name collisions.** The kernel import name is the `server`
  value. On a custom `PROMETH_KERNEL_PYTHON` that already has an unrelated PyPI
