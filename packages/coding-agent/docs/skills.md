> Prometh can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that Prometh loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

Prometh implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent IPython kernel.
...
- [Creating Skills with Prometh](#creating-skills-with-prometh)
...
- Built-in: `skills/` shipped with the prometh package (lowest precedence)

Discovery rules:
- In `~/.prometh/skills/` and `.prometh/skills/`, direct root `.md` files are discovered as individual skills
...
Prometh ships with built-in skills that load by default:
...
```bash
export PROMETH_WEBSEARCH_TIMEOUT=45
export PROMETH_WEBSEARCH_NUM_RESULTS=5
```
...
```python
print(await websearch("latest Prometh release"))
```
...
For project-level Claude Code skills, add to `.prometh/settings.json`:
...
1. At startup, Prometh scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, Prometh exposes `web_search` in IPython. If the module defines `run()`, the module is wrapped as an async callable:
...
Python skills are installed editable into the kernel venv during kernel setup. By default this is `~/.prometh/kernel-venv`; set `PROMETH_KERNEL_VENV` to override it. If `pyproject.toml` changes, Prometh rebuilds the kernel venv so dependency changes are picked up.

If you set `PROMETH_KERNEL_PYTHON`, Prometh does not install packages into that environment. The Python must already have `ipykernel`, `prometh-runtime`, and the default runtime packages installed. Missing Python skill imports are disabled with a warning and calling the skill raises a `RuntimeError`.
...
## Creating Skills with Prometh

Prometh ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:

```text
Create a project Python-backed skill named release-audit in
.prometh/skills/release-audit. It should expose
...
1. **Scope:** use `.prometh/skills/<name>/` for a project skill committed with the repository, or `~/.prometh/skills/<name>/` for a personal skill.
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh Prometh session after adding a Python-backed skill so kernel setup can install and import the package.
...
Prometh validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
