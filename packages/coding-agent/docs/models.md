...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.prometh/models.json`.
...
For `models.json`, shell commands are resolved at request time. Prometh intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and Prometh cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps Prometh thinking levels to provider values and marks unsupported levels (see below) |
...
Current behavior:
- `/model` and `prometh model list` list entries by model `id`.
- The configured `name` is used for model matching and detail/status text.

### Thinking Level Map

Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are Prometh thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`.
...
By default Prometh sends per-tool `eager_input_streaming: true`. If a proxy or Anthropic-compatible backend rejects that field, set `supportsEagerToolInputStreaming` to `false`. Prometh will omit `tools[].eager_input_streaming` and send the legacy `fine-grained-tool-streaming-2025-05-14` beta header for tool-enabled requests instead.
