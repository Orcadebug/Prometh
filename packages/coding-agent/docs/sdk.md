> Prometh can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to Prometh's capabilities. Use it to embed Prometh in other applications, build custom interfaces, or integrate with automated workflows.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.prometh", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.prometh/extensions/`)
- Project skills:
  - `.prometh/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.prometh/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.prometh/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.prometh/auth.json and ~/.prometh/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
- If you omit `tools`, Prometh automatically creates them with the correct `cwd`
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.prometh/extensions/`, `.prometh/extensions/`, and `settings.json` extension sources.
...
1. Global: `~/.prometh/settings.json`
2. Project: `<cwd>/.prometh/settings.json`
