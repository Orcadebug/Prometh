...
const OWNED_WORKER_ENV = "PROMETH_INTERNAL_OWNED_WORKER";
const OWNED_RECOVERY_DESCRIPTOR_ENV = "PROMETH_INTERNAL_OWNED_RECOVERY_DESCRIPTOR";
const OWNED_PROFILE_ENV = "PROMETH_INTERNAL_OWNED_PROFILE";
...
export async function runOwnedSessionWorkerFrontend(
	...
	const recoveryDescriptorPath = join(tmpdir(), `prometh-owned-${process.pid}-${randomUUID().slice(0, 12)}.json`);
	...
	const anonymousRpcIdPrefix = `prometh-owned-${randomUUID()}`;
...
export async function maybeRunOwnedSessionWorkerFrontend(
	...
	if (!forceLegacyFrontend && process.env.PROMETH_INTERNAL_LEGACY_OWNED_WORKER_FRONTEND !== "1") {
		return false;
	}
