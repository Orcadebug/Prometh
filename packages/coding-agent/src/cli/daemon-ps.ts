...
/**
 * `daemon ps` discovers every prometh daemon on the machine, not just the
 * one on a single socket. Discovery has two sources merged by socket path:
 *
 *  1. The OS list of listening unix sockets owned by a prometh process
...
/** Parse `ss -lxp` output into the prometh daemons listening on unix sockets. */
...
export function parsePromethProcessIds(stdout: string, appName: string): number[] {
	const pids: number[] = [];
	for (const line of stdout.split("\n")) {
...
function scanListeningDaemons(): DiscoveredDaemonProcess[] {
	...
	if (!ps.error && ps.status === 0 && typeof ps.stdout === "string") {
		const pids = parsePromethProcessIds(ps.stdout, APP_NAME);
		if (pids.length > 0) {
	...
	return daemons.map((daemon): ReapAction => {
		...
		if (daemon.status === "unreachable") {
			if (!force || daemon.pid === undefined) {
				return { kind: "skip", daemon, reason: 'unreachable; use "prometh shutdown --force" to stop it' };
			}
...
export async function runShutdownAll(json: boolean, force: boolean): Promise<void> {
	...
	switch (planShutdownConfirmation(daemons.length, json, force, process.stdin.isTTY)) {
		case "json-error":
				...
				JSON.stringify(
					{
						...
						failed: daemons.map(({ socketPath }) => ({
							socketPath,
							reason: 'confirmation required; use "prometh shutdown --force --json"',
						})),
					},
					null,
					2,
			...
			return;
		case "tty-error":
			throw new Error('Shutdown requires confirmation in an interactive terminal. Use "prometh shutdown --force".');
		case "prompt": {
