...
export const PROMETH_BUILD_ID_ENV = "PROMETH_BUILD_ID";
export const PROMETH_LAUNCHER_PATH_ENV = "PROMETH_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[PROMETH_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[PROMETH_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
