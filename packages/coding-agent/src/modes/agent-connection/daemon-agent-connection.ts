...
export class DaemonAgentConnection implements AgentConnection {
	...
	async manageHeartbeat(
		...
		if (!this.client.supportsServerCapability("heartbeat_management")) {
			throw new Error("Heartbeat management requires a newer Prometh daemon.");
		}
		try {
	...
	private formatDaemonSessionClosedError(reason: DaemonSessionClosedReason): string {
		const explanation: Record<DaemonSessionClosedReason, string> = {
			...
			shutdown:
				"The Prometh daemon shut down while this window was attached. The session transcript remains saved; restart Prometh and reopen it from Agents View.",
			completed:
			...
			update:
				"The Prometh daemon restarted for an update, but this window did not restore automatically. The session transcript remains saved; restart Prometh and reopen it from Agents View.",
		};
		return `${explanation[reason]} ${this.formatDaemonDiagnosticContext()}`;
	}

	private formatDaemonConnectionClosedError(error: Error): string {
		return `Lost connection to the Prometh daemon. Cause: ${formatErrorSentence(error)} The session transcript remains saved; restart Prometh or reopen the session from Agents View. ${this.formatDaemonDiagnosticContext()}`;
	}

	private formatUpdateReconnectError(error: unknown): string {
		return `The Prometh daemon restarted for an update, but this window could not reconnect to its restored session before the recovery timeout expired. Last error: ${formatErrorSentence(error)} The session transcript remains saved; restart Prometh and reopen it from Agents View. ${this.formatDaemonDiagnosticContext()}`;
	}
	...
	private reconnectAfterUpdate(): Promise<void> {
		...
		void this.emit({
			...
			status: "reconnecting",
			error: "The Prometh daemon is restarting for an update.",
		});
