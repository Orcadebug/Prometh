 ...
 *
 * prometh acts as an ACP agent over NDJSON on stdio, driving an
 * `AgentConnection` in-process. It deliberately does not shell out to RPC mode
 * and translate: prometh's differentiators (IPython-only tools, subagents,
...
/**
 * Split ACP prompt blocks into the text and images prometh accepts.
 *
...
export async function runAcpModeWithConnection(
	...
	const handle = acp
		...
		.onRequest("initialize", async () => ({
			...
			agentInfo: { name: "prometh", title: "Prometh", version: VERSION },
			// Advertise prometh extras under a namespaced key: ACP reserves
		...
		.onRequest("session/new", async (ctx: any) => {
			...
			if (session) {
				throw new Error(
					"prometh ACP mode hosts one session per connection; " +
						"start another prometh process for a second session",
				);
			}
			// prometh's cwd is fixed at startup by the session it was launched
			...
			// Subscribe for the session lifetime, not per prompt turn: prometh
			...
			const unsubscribe = connection.subscribe((event) => {
				...
				if (event.type === "heartbeats_changed") {
					notify({ sessionUpdate: "session_info_update", _meta: promethMeta({ heartbeatsChanged: true }) });
					return;
				}
			...
			return {
				sessionId,
				...(cwdMismatch ? { _meta: promethMeta({ cwd: cwdMismatch }) } : {}),
			};
		})
		.onRequest("session/prompt", async (ctx: any) => {
			...
			try {
				...
				if (failure && !abort.signal.aborted) {
					throw new Error(`prometh turn failed: ${failure}`);
				}
