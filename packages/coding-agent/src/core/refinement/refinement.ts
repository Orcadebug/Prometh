...
export const REFINEMENT_CUSTOM_TYPE = "prometh.refinement";
...
const REFINEMENT_SYSTEM_PROMPT = `You are Prometh's /refine continual harness subsystem.
...
skills, and subagent specs that lets Prometh improve reusable behavior
...
- The default editable continual harness store is local to the current Prometh session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Prometh's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this Prometh session. Global continual harness entries persist across Prometh sessions.",
