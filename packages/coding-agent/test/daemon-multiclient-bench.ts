...
function socketPathForRun(): string {
	...
	if (process.platform === "win32") {
		return `\\\\.\\pipe\\prometh-multiclient-bench-${process.pid}-${socketCounter}`;
	}
	return `/tmp/prometh-multiclient-${process.pid}-${socketCounter}.sock`;
}
...
const streamCorpus = repeatedText(STREAM_CHARS, "Prometh daemon multi-client streaming benchmark. ");
...
function createAttachResponseForMessages(
	...
	const state = {
		...
		sessionDir: "/tmp/prometh-benchmark/sessions",
		...
		for (let offset = 0, sequence = 1; offset < streamCorpus.length; sequence++) {
			...
			const compact = createCompactAssistantDelta({
				...
				meta: {
					...
					protocol: { name: "prometh.daemon", version: 2 },
					activeSessionId: "active-benchmark",
					sequence,
...
async function runAttachV2(
	...
	const cacheRoot = await mkdtemp(join(tmpdir(), "prometh-attach-v2-bench-"));
...
async function loadSessionFixture(sessionFile: string): Promise<{
	...
	const tempDir = await mkdtemp(join(tmpdir(), "prometh-session-bench-"));
...
function printResults(
	...
	console.log("Prometh daemon multi-client benchmark");
	console.log(details);
	console.table(
		results.map((result) => ({
...
function printSessionLoad(sessionFile: string, result: SessionLoadResult): void {
	console.log("Prometh real-session benchmark");
...
async function generateSessionFixture(sizeMiB: number): Promise<GeneratedSessionFixture> {
	const directory = await mkdtemp(join(tmpdir(), `prometh-generated-${sizeMiB}mib-`));
