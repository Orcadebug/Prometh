...
describe("kernel bootstrap", () => {
	beforeEach(async () => {
		...
		tempDir = mkdtempSync(join(tmpdir(), "prometh-kernel-bootstrap-"));
		process.env.HOME = tempDir;
		process.env.PATH = originalEnv.PATH ?? "";
		delete process.env.PROMETH_KERNEL_PYTHON;
		delete process.env.PROMETH_KERNEL_VENV;
	...
	it("returns the configured kernel venv directory", () => {
		...
		process.env.PROMETH_KERNEL_VENV = venv;

		expect(getKernelVenvDir()).toBe(venv);
	});

	it("bootstraps a missing venv with uv, ipykernel, prometh-runtime, and default extra packages", async () => {
		...
		expect(log).toContain("prometh-runtime");
	...
	it("installs sibling Python skill dependencies when package and directory names differ", async () => {
		...
		writeFileSync(
			...
			`[project]
name = "prometh-skill-attach-image"
version = "0.1.0"
`,
		);
		const dependentSkill = createPythonSkillWithDependency("orchestration-heartbeat", "prometh-skill-attach-image");
		process.env.PROMETH_KERNEL_VENV = venv;
...
`,
	...
	it("uses PROMETH_KERNEL_PYTHON as an override contract", async () => {
		...
		process.env.PROMETH_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).resolves.toBe(overridePython);
	});

	it("allows PROMETH_KERNEL_PYTHON missing Python skill imports", async () => {
		const overridePython = join(tempDir, "override-python");
		const pythonSkill = createPythonSkill();
		writeFakePython(overridePython, ["ipykernel", "rlm", ...DEFAULT_RLM_EXTRA_IMPORT_NAMES]);
		process.env.PROMETH_KERNEL_PYTHON = overridePython;
	...
	it("rejects PROMETH_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects PROMETH_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, ["ipykernel"]);
		process.env.PROMETH_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/current prometh-runtime with callable rlm\.run/);
	});

	it("rejects PROMETH_KERNEL_PYTHON with a legacy harness API", async () => {
		const overridePython = join(tempDir, "override-python");
		writeExecutable(
			overridePython,
			[
	...
	it("fails an invalid PROMETH_KERNEL_PYTHON without bootstrapping", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, []);
		process.env.PROMETH_KERNEL_PYTHON = overridePython;
