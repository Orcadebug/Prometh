...
describe("Prime Inference auth", () => {
	...
	beforeEach(() => {
		...
		originalTraceBaseUrl = process.env.PROMETH_TRACES_BASE_URL;
		delete process.env.PROMETH_TRACES_BASE_URL;
	});
	...
	afterEach(() => {
		if (originalTraceBaseUrl === undefined) {
			...
			process.env.PROMETH_TRACES_BASE_URL = originalTraceBaseUrl;
		}
	...
	it("checks Prometh trace access with Prime whoami permissions", async () => {
		const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
			expect(getUrl(input)).toBe("https://prime-api.example/api/v1/user/whoami");
			expect(init?.method).toBe("GET");
			expect(getAuthorization(init)).toBe("Bearer prime-key");
		...
		await expect(
			checkPromethTracesAccess("prime-key", "https://prime-api.example", { fetchFn: fetchMock }),
		).resolves.toEqual({ ok: true });
		expect(fetchMock).toHaveBeenCalledOnce();
	});
	...
	it("validates imported Prime CLI trace credentials against the production trace API by default", async () => {
		...
		const result = await loginPromethTraces({ onAuth }, { configPath, fetchFn: fetchMock, requestTimeoutMs: 1000 });
	...
	it("validates imported Prime CLI trace credentials against PROMETH_TRACES_BASE_URL", async () => {
		process.env.PROMETH_TRACES_BASE_URL = "https://trace-api.example/api/v1";
		writeFileSync(
			configPath,
			JSON.stringify({
				api_key: "prime-cli-key",
	...
	it("requests agent trace scope during Prometh trace browser login", async () => {
		process.env.PROMETH_TRACES_BASE_URL = "https://prime-api.example/api/v1";
		writeFileSync(
			configPath,
			JSON.stringify({
				base_url: "https://prime-api.example",
		...
		const result = await loginPromethTraces(
			{
				onAuth,
			},
			{ configPath, fetchFn: fetchMock, pollIntervalMs: 0, requestTimeoutMs: 1000 },
