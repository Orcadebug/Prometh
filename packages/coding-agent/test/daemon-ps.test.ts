...
describe("parseSsListeners", () => {
	const stdout = [
		...
		'u_str LISTEN 0      511    /tmp/custom.sock 10147608 * 0 users:(("prometh",pid=1234,fd=22))',
		'u_str LISTEN 0      511    /tmp/prometh-1000/daemon.sock 79453846 * 0 users:(("prometh",pid=5678,fd=24))',
		'u_str LISTEN 0      4096   /run/dbus/system_bus_socket 123 * 0 users:(("dbus-daemon",pid=900,fd=3))',
		'u_str ESTAB  0      0      /tmp/other.sock 456 * 0 users:(("prometh",pid=4321,fd=9))',
		"",
	].join("\n");

	it("extracts socket + pid for prometh LISTEN sockets only", () => {
		const daemons = parseSsListeners(stdout, "prometh");
		expect(daemons).toEqual([
			{ pid: 1234, socketPath: "/tmp/custom.sock" },
			{ pid: 5678, socketPath: "/tmp/prometh-1000/daemon.sock" },
		]);
	});
...
describe("parsePromethProcessIds", () => {
	it("finds process.title names even when lsof reports the executable as node", () => {
		const stdout = [
			"  123 node prometh --mode daemon",
			"  456 prometh prometh",
			"  789 /usr/local/bin/prometh prometh",
			"  900 node unrelated.js",
			"",
		].join("\n");
		expect(parsePromethProcessIds(stdout, "prometh")).toEqual([123, 456, 789]);
	});
});
...
describe("planReap", () => {
	...
	it("only kills unreachable daemons with --force", () => {
		...
		expect(skipped.kind === "skip" ? skipped.reason : "").toContain("prometh shutdown --force");
