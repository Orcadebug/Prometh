...
function emitHello(
	...
	socket.emit(
		...
		`${JSON.stringify({
			type: "daemon_hello",
			socketPath: "/tmp/prometh.sock",
			protocol: { name: "prometh.daemon", version },
			schemaRevision,
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities,
...
describe("DaemonClient", () => {
	...
	it("allows connect retry after the socket emits an error before connecting", async () => {
		const client = new DaemonClient("/tmp/prometh-missing.sock");
		...
		expect(firstError.message).toContain("Failed to connect to the Prometh daemon: initial connect failed.");
		expect(firstError.message).toContain("Socket: /tmp/prometh-missing.sock.");
		...
		await expect(secondAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Failed to connect to the Prometh daemon: retry reached socket."),
		});
	});

	it("allows connect retry after the initial connection times out", async () => {
		vi.useFakeTimers();
		const client = new DaemonClient("/tmp/prometh-slow.sock");
		...
		const timeoutRejection = expect(firstAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Timed out after 5ms connecting to the Prometh daemon."),
		});
	...
	it("captures the daemon hello greeting for version checks", async () => {
		const client = new DaemonClient("/tmp/prometh.sock");
		...
		const hello = {
			type: "daemon_hello",
			socketPath: "/tmp/prometh.sock",
			protocol: { name: "prometh.daemon", version: 1 },
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities: [],
		};
	...
	it("serializes activeSessionId for session commands", async () => {
		...
		expect(envelope).toMatchObject({
			...
			protocol: { name: "prometh.daemon", version: DAEMON_PROTOCOL_VERSION },
	...
	it("includes command, socket, and log context when a request is made while disconnected", async () => {
		...
		await expect(request).rejects.toMatchObject({
			message: expect.stringContaining(
				'Cannot send daemon command "list" because the Prometh daemon is not connected.',
			),
		});
		await expect(request).rejects.toMatchObject({
			message: expect.stringContaining("Socket: /tmp/prometh.sock."),
		});
		await expect(request).rejects.toMatchObject({
	...
	it("notifies listeners when a connected daemon socket closes", async () => {
		...
		expect(closed[0]?.message).toContain("Connection to the Prometh daemon closed.");
		expect(closed[0]?.message).toContain("Socket: /tmp/prometh.sock.");
