...
describe("herdrAgentStateExtension", () => {
	...
	it("reports lifecycle state to the herdr socket", async () => {
		...
		process.env.PROMETH_CODING_AGENT_DIR = tempDir;
		...
		expect(requests[0]?.params.agent).toBe("prometh");
		...
		expect(requests[3]?.params.agent).toBe("prometh");
	});
