...
vi.mock("../src/package-manager-cli.js", () => ({
	...
	isSelfUpdateSource: (source: string) => source === "self" || source === "pi" || source === "prometh",
}));
...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("prometh attach <agent>"));
	});
	...
	it("separates Prometh updates from package updates", async () => {
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		for (const target of ["self", "--self", "prometh"]) {
			await handlePublicCommand(["update", target]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "prometh update [--force]"'));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "prometh package update [source]"'));
	});
	...
	it("rejects self-update aliases on the package update path", async () => {
		for (const source of ["self", "pi", "prometh"]) {
			await handlePublicCommand(["package", "update", source]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "prometh update"'));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "prometh package remove"'));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("prometh package list"));
	});
	...
	it("rejects the old daemon hierarchy with migration guidance", async () => {
		...
		expect(process.exitCode).toBe(1);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Run "prometh help"'));
	});

	it("shows migration guidance when help targets removed commands", async () => {
		const cases: Array<[path: string[], hint: string]> = [
			[["daemon"], 'Run "prometh help"'],
			[["install"], 'Use "prometh package install"'],
			[["remove"], 'Use "prometh package remove"'],
			[["uninstall"], 'Use "prometh package remove"'],
			[["manage"], 'Use "prometh agents"'],
			[["app", "update"], 'Use "prometh update"'],
		];
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining("prometh list [--all] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining("prometh doctor [--fix] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining("prometh package install <source>"));
