...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".prometh", "prompts");
			...
			const projectSkillDir = join(cwd, ".prometh", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".prometh", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".prometh", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".prometh"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".prometh", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".prometh", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".prometh", "extensions");
...
Content`,
	...
	describe("bundled skills", () => {
		...
		it("should let a project skill override the bundled websearch skill", async () => {
			const projectSkillDir = join(cwd, ".prometh", "skills", "websearch");
			mkdirSync(projectSkillDir, { recursive: true });
			writeFileSync(
