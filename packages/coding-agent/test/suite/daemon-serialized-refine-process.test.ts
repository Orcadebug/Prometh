 ...
 *     PROMETH_INTERNAL_DAEMON_WORKER=1 so the auto-spawned supervisor
...
describe("Real-process serializedRefine — JSON mode", () => {
	it("daemon supervisor scrubs inherited worker env, checkpoint applies refine_complete before agent_end", async () => {
		const root = mkdtempSync(join(tmpdir(), "prometh-process-json-refine-"));
		tempRoots.add(root);
		...
		const result = await runCli(
			...
			],
			{
				agentDir,
				environment: {
					PROMETH_TEST_EVENT_LOG: eventLogPath,
