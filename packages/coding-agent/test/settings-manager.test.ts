...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".prometh"), { recursive: true });
	});
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".prometh", "settings.json");
		...
		it("should report a new project error when saving after the load error was drained", async () => {
			const settingsPath = join(projectDir, ".prometh", "settings.json");
		...
		it("drains only the requested scope", () => {
			...
			writeFileSync(join(projectDir, ".prometh", "settings.json"), "{ invalid project json");
			const manager = SettingsManager.create(projectDir, agentDir);
	...
	describe("project settings directory creation", () => {
		it("should not create .prometh folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .prometh folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .prometh folder that beforeEach created
			rmSync(join(projectDir, ".prometh"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .prometh folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".prometh"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .prometh folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .prometh folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .prometh folder should NOT exist yet
			expect(existsSync(join(projectDir, ".prometh"))).toBe(false);
			...
			// Now .prometh folder should exist
			expect(existsSync(join(projectDir, ".prometh"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".prometh", "settings.json"))).toBe(true);
		});
	});
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".prometh", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
	...
	describe("mcpServers", () => {
		...
		it("merges global and project mcpServers, project winning per key", () => {
			...
			writeFileSync(
				join(projectDir, ".prometh", "settings.json"),
				JSON.stringify({
					mcpServers: {
	...
	describe("idle worker eviction", () => {
		...
		it("reads and writes the global daemon policy without project overrides", async () => {
			...
			writeFileSync(join(projectDir, ".prometh", "settings.json"), JSON.stringify({ idleEvictionMinutes: 30 }));
			const manager = SettingsManager.create(projectDir, agentDir);
