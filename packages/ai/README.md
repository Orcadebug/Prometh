...
<h1 align="center">Prometh AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the Prometh package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
```bash
npm install prometh-ai
```

TypeBox exports are re-exported from `prometh-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'prometh-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'prometh-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'prometh-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'prometh-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'prometh-ai';
...
```typescript
import { getModel, stream } from 'prometh-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'prometh-ai';
...
```typescript
import { Model, stream } from 'prometh-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Prometh thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'prometh-ai';
...
```typescript
import { getModel, complete, Context } from 'prometh-ai';
...
```typescript
import { Context, getModel, complete } from 'prometh-ai';
...
- OAuth login flows are not supported in browser environments. Use the `prometh-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'prometh-ai';
...
```bash
npx prometh-ai login              # interactive provider selection
npx prometh-ai login anthropic    # login to specific provider
npx prometh-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `prometh-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'prometh-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'prometh-ai/oauth';
...
```typescript
import { getModel, complete } from 'prometh-ai';
import { getOAuthApiKey } from 'prometh-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `prometh-ai`
